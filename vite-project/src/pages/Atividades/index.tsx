...
const Atividades = () => {
  ...
  const handleGetAtividades = async (id: number, filtro: string, page = 0, size = 10) => {
    try {
      ...
      setAtividades(response.data.content); // Atividades da página atual
      setInfoPaginacao({
        currentPage: response.data.pageable.pageNumber + 1, // Zero-based para one-based
  ...
  return (
    <div>
      ...
      <ContainerItem>
        ...
        <Paginacao
  currentPage={infoPaginacao.currentPage} // Página atual
  totalItems={infoPaginacao.totalItems}   // Total de registros
  itemsPerPage={infoPaginacao.itemsPerPage} // Tamanho da página
  onPageChange={(page) => handleGetAtividades(userData.idUsuario, filtro, page - 1)} // Subtrai 1 para alinhar com zero-based
/>
      </ContainerItem>
    </div>
  );
